function card(a,opts){
  ...
  return "<div class='card'><div class='apt' onclick='openApt(\""+id+"\")'><div class='timebox'>"+esc((a.time||"--:--").slice(0,5))+"<small>"+(s?s.minutes+" min":"")+"</small></div><div style='flex:1'><div class='name'>"+esc(c?c.name:"Cliente")+"</div><div class='muted'>"+esc(s?s.name:"Servizio")+"</div><div style='margin-top:8px;display:flex;gap:6px;flex-wrap:wrap'><span class='chip'>"+st+"</span><span class='chip "+(due>0.01?"debt":"paid")+"'>"+(due>0.01?"Deve "+euro(due):euro(a.price||0))+"</span></div></div></div>"+actions+"</div>";
}
function vToday(){
  ...
  return html+next.map(function(a){return card(a,{today:1})}).join("")+(done.length?"<p class='muted' style='margin:8px 4px'>Gia fatte</p>"+done.map(function(a){return card(a)}).join(""):"");
}
function vAgenda(){
  ...
  for(let d=1;d<=dim;d++){
    ...
    cells+="<button type='button' class='"+cls+"' onclick='pickDay(\""+iso+"\")'>"+d+"</button>";
  }
  const list=apts(selectedDate);
  return "<div class='card'><div class='row' style='margin-bottom:10px'><button type='button' class='btn btn-ghost btn-sm' onclick='shiftM(-1)'>←</button><strong style='text-transform:capitalize'>"+first.toLocaleDateString("it-IT",{month:"long",year:"numeric"})+"</strong><button type='button' class='btn btn-ghost btn-sm' onclick='shiftM(1)'>→</button></div><div class='cal-grid'>"+["L","M","M","G","V","S","D"].map(function(x){return "<div class='dow'>"+x+"</div>"}).join("")+cells+"</div></div><p class='muted' style='margin:4px 4px 10px;text-transform:capitalize'>"+ndl(selectedDate)+"</p>"+(list.length?list.map(function(a){return card(a)}).join(""):"<div class='card empty'><p>Nessun appuntamento.</p></div>");
}
function pickDay(iso){selectedDate=iso;render()}
...
function vMoney(){
  ...
  if(cred.length) debts=cred.map(function(x){return "<div class='list-item row' onclick='openClient(\""+x.c.id+"\")'><div class='name'>"+esc(x.c.name)+"</div><span class='chip debt'>"+euro(x.b)+"</span></div>"}).join("");
  return "<div class='card'><div class='muted'>Oggi ho preso</div><div class='money' style='font-size:36px;margin:4px 0'>"+euro(todayPaid)+"</div></div><div class='card'><div class='muted'>Questo mese</div><div class='money' style='font-size:32px;margin:4px 0'>"+euro(inc)+"</div></div><div class='card'><h3>Mi devono</h3>"+debts+"</div><div class='card'><h3>Copia di sicurezza</h3><div class='grid2'><button type='button' class='btn btn-soft' onclick='exp()'>Salva copia</button><button type='button' class='btn btn-ghost' onclick='document.getElementById(\"imp\").click()'>Ripristina</button></div><input id='imp' type='file' accept='application/json' class='hidden' onchange='imp(event)'></div>";
}
